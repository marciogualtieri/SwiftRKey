...
The objectives of this notebook are basic data cleaning, exploratory data analysis and model design for a keyboard typing predicting app, (i.e., capable of predicting what's the next word given the currently typed words).
...
The next step is going to be the creation of a R script to produce and save to disk an improved model of the same design (spoiler: the model consists of a transition matrix which represents a [Markov process](https://en.wikipedia.org/wiki/Markov_chain)). The model produced in this notebook is only a proof of concept to verify design choices. In a bare bones R script (without exploratory data analysis code) we can better utilize the available resources (memory and CPU) and work with a larger corpus of text to improve our probability estimates.
...
create_sample_files("./data/final/en_US", "./data/samples", 0.005, 0.3)
```
...
is_english_dt <- one_ngrams_dt[, is_english := is_english(ngram)]
...
We are going to show the calculation steps for the example: $P_{bo}(\text{"word"}|\text{"bird is the"})$.
...
Supposing that $C(\text{"is the word"}) \leq k$, we already know how to calculate that (previous section).
...
We are going to need a sub-set of n-grams of different sizes. I'm creating a function to generate an amalgate of n-grams with different sizes from the individual n-gram data-sets:
...
build_processed_ngram_frequencies <- function(frequencies_dt) {
    data_dt <- data.table(frequencies_dt)
    data_dt[, ngram := strsplit(ngram, split = " |'")]
    data_dt[, ngram_length := sapply(ngram, length)]
    data_dt[, history := sapply(ngram, extract_history)]
    data_dt[, word := sapply(ngram, extract_word)]
    data_dt[, ngram := sapply(ngram, paste, collapse = " ")]
}
...
Given that $\beta_i$ and $\sum_{i} \alpha_i$ are probabilities, they need to be smaller or equal to one. In practice though, we are working with estimates obtained from the text corpus and estimations will be as good as our data, thus the calculations might result in values greater than one.

Another issue is that computers introduce rounding errors. For this reason, we are limiting the value of $\alpha_i$ and $\beta_i$ to one, that is, if the estimate $\alpha_i$ and $\beta_i$ exceeds one, we assume they are actually equal to one.

#### Computing a Markov Chain Trainsition Matrix

Now that we can compute Katz's probabilities, we will create a transition matrix, which is a representation of a [Markov Chain](https://en.wikipedia.org/wiki/Markov_chain):
...
Given that we are going to ultimately load the model into memory in the keyboard typing prediction app and that shinyapps.io has a limitation (for the free account) of 1GB, let's also take a look at the model's size
...
```{r}
suggestions(model, "new")
```

The suggestions make sense, so it seems like we are in good shape. In the next section, we are going to evaluate how good our model is in a more quantitative manner.

### Model Perplexity

We are going to use [perplexity](https://en.wikipedia.org/wiki/Perplexity) as a measure for model performance:
...
First we would need a function to extract conditional probabilities from the model:
...
We are going to sample again from the corpus and create a test corpus:

```{r}
test_corpus <- VCorpus(DirSource("./data/samples/testing"), readerControl = list(language = "en_US"))
test_corpus <- clean_corpus(test_corpus)
test_corpus <- tm_map(test_corpus, removeWords, curse_words)
...
Here we calculate perplexity for different N-Gram sizes:
...
```{r, eval = FALSE}
read(file = "./data/model.rda")
```

An object named `model` should be added to the environment. That's how we're going to load our model in the app.

Depending on the environment where the app will be deployed, there will different limits for memory and resources.

Given that we are computing the transition matrix (model) probabilities before hand we are somewhat shielded from performance issues (the app using the model
doesn't have to perform any computation, only retrieve the top word predictions to return), but we still might have limitations regarding the biggest model that we can fit into the available memory.
...
We are going to store our model as a Markov Chain Transition Matrix.
...
The model is a transition matrix with size ($\text{Number of History N-Grams} \times \text{Number of Predicted Words}$). This matrix size is the number of parameters we need.

We are going to smooth probabilities using Good-Turing estimation.

We are going to evaluate our model by calculating perplexity.

We are going to use Katz's Back-off to estimate the probability of unobserved n-grams.
